...
class BanReservedValidator < ActiveModel::EachValidator
  WORDS = %w[
    ...
    api wiki auth session register login logout sign_up sign_in sign_out
  ...
  def validate_each(record, attribute, value)
    if (WORDS + WORDS.map(&:pluralize)).include?(value)
      record.errors[attribute] <<
        (options[:message] || I18n.t('activerecord.errors.messages.reserved'))
    end
  end
end
